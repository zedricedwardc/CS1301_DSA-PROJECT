...
void FlappyBird::gotoxy(int x, int y) {
    CursorPosition.X = x;
    CursorPosition.Y = y;
    SetConsoleCursorPosition(console, CursorPosition);
}

void FlappyBird::setcursor(bool visible, DWORD size) {
    if (size == 0)
        size = 20;
...
void FlappyBird::genPipe(int ind) {
    gapPos[ind] = 3 + rand() % 14;
}

void FlappyBird::drawPipe(int ind) {
...
void FlappyBird::erasePipe(int ind) {
    if (pipeFlag[ind] == true) {
        for (int i = 0; i < gapPos[ind]; i++) {
            gotoxy(WIN_WIDTH - pipePos[ind], i + 1); cout << "   ";
        }
...
void FlappyBird::Menu()
{
    ...
    do {
        ...
        if (op == '1') play();
        else if (op == '2'){ instructions(); return Menu();}
        else if (op == '3') return;
    } while (op == '1');
}